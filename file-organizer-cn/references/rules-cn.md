...
| 代码与数据 | py、js、ts、html、css、json、yaml、sql、ipynb 等；默认跳过 |
| 其他 | 未命中上述规则的普通文件；默认跳过 |

扩展名匹配不区分大小写。工具只按文件类型归类，不判断文件内容，也不自动删除重复文件。

## 计划锁定

预览会根据目标目录、排除分类、文件路径、大小和修改时间生成 `plan_id`。正式整理必须提供该编号；只要待移动文件发生变化，工具就拒绝使用旧计划。计划编号不是安全密码，而是一致性校验凭据。

## 报告隐私

HTML 报告完全离线，不加载网络资源。默认隐藏完整路径和文件名，仅保留扩展名、分类、数量、大小和计划编号；使用 `--reveal-names` 才显示文件名。报告必须保存在待整理文件夹之外，不会覆盖同名文件。
