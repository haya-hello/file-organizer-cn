...
description: 跨平台的中文文件整理助手，先预览文件夹中的分类方案，再经用户明确确认后移动文件，并保留可撤销记录。用户提到整理下载文件夹、桌面太乱、文件自动归类、查找大文件、恢复刚才的整理，或希望安全清理文件但不想误删时使用。适用于 Windows、macOS 和 Linux；不用于删除文件。
---

# 文件整理助手

把“整理文件夹”拆成可检查、可确认、可撤销的流程。默认只查看目标文件夹第一层，不进入子文件夹，不删除文件，也不覆盖同名文件。

## 必须遵守的安全顺序

1. 确认目标文件夹。用户未指定时，优先使用 `downloads`，但要在回复中写出解析后的绝对路径。
2. 先运行 `preview`。即使用户说“帮我整理”，第一次也只能预览，不得直接移动。
3. 用普通话概括将移动多少文件、主要分类、跳过了什么，并给出少量示例。
4. 等用户看过预览并明确同意后，才能运行带 `--confirm` 的 `organize`。
...
读取 JSON 中的 `total_files`、`total_size_bytes`、`categories`、`moves` 和 `skipped`。向用户展示：
...
使用一句简短问题，例如：“预览显示会移动 36 个文件到该目录下的‘已整理’文件夹，不会删除或覆盖文件。现在执行吗？”
...
```bash
python "<SKILL_DIR>/scripts/file_organizer.py" organize "<目标文件夹>" --confirm --json
```
